...
public class FaseWebController {
	...
	public ModelAndView faseProduto(@PathVariable("idProduto") Long idProduto, HttpServletRequest request,
		...
		List<ProdutoFase> produtosFase = produtoFaseService.list().stream()
				.filter(pf -> pf.getProduto().getCodProduto().equals(idProduto)).collect(Collectors.toList());
		modelAndView.addObject("fasesProduto", produtosFase);
		modelAndView.setViewName("fases/produto");
		return modelAndView;
	}

	@GetMapping("**/fase/addMP/{codFaseProd}")
	public ModelAndView addMP(@PathVariable("codFaseProd") Long idFaseProd, HttpServletRequest request,
		...
		List<ProdutoFaseMat> listProdutoFase = new ArrayList<>();
		listProdutoFase.add(new ProdutoFaseMat());
		pf.setProdutoFasemat(listProdutoFase);
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.addObject("produto", pf.getProduto());
		modelAndView.addObject("fase", pf.getFase());
		modelAndView.addObject("fasesProduto", pf);
		modelAndView.addObject("materias", materiaPrimaService.list());
		modelAndView.setViewName("fases/addMP");
		return modelAndView;
	}

	@PostMapping("/fases")
	public ModelAndView save(@Valid Fase fase, @Valid Produto produto, @Valid ProdutoFase produtoFase, BindingResult result,
